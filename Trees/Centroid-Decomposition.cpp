 ...
 * build - builds the centroid decomposition -> O(n log n). 
*/
...
/* Save list of centroid-ancestors and distances to them for each vertex
vector<vector<pair<int,int>>> anc(n+1);
auto dfs_dist = [&](int v, int v0, int c, int cur, auto&& dfs_dist)->void{
    for(auto u : g[v]){
        if(u==v0 || used[u]) continue;
